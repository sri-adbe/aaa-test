...
/**
 * Transformer: AAA (ace.aaa.com) section breaks and section metadata.
 * Template "deals" has 13 sections; selectors come from page-templates.json
 * (DOM-verified during page analysis). Inserts <hr> before every non-first
 * section in beforeTransform (while section elements still exist, before
 * parsers replace them), and Section Metadata blocks in afterTransform for
 * any styled section. All sections currently have style:null, so only breaks
 * are inserted; the metadata path is retained for correctness/reuse.
 */
